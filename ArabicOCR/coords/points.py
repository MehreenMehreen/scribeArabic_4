...
def is_near_vertical(poly_points, thresh=15):
    
    minX, minY, maxX, maxY = get_max_min_polygon(poly_points)
    
    if (maxY - minY) > (maxX - minX):
        return True
    
    
    angle = polygon_orientation(poly_points)
    
    if abs(angle) <= 90 + thresh and abs(angle) >= 90 - thresh:
        return True
    return False


# Takes care of vertically oriented text as well as bottom up/top down + upside down text
def get_baseline_main(poly_points, right_to_left=True, top_down=True, upside_down=False):  
    
    # First check if we have near vertical line
    is_vertical = is_near_vertical(poly_points)
    if is_vertical:
       
        # First reverse coords
        poly = [(y, x) for (x, y) in poly_points]
        baseline = get_baseline_chunks(poly)
        # reverse again
        baseline = [(x, y) for (y, x) in baseline]
        
    else:
        baseline = get_baseline_chunks(poly_points)
