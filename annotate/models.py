...
class imageFiles():
    ...
    def load_files(self, directory='datasets', prefix_to_add=""):
        file_list = []
        path = os.path.join(settings.DATASET_ROOT, directory)
...
class directories():
    ...
    def load_directories(self, root_dir=None, task="annotate", 
                            user="", include_submitted=False):

        if root_dir is None:
            root_dir = os.path.join(settings.DATASET_ROOT, settings.DATASET_PATH)
        files = imageFiles()
        self.total_files = 0
