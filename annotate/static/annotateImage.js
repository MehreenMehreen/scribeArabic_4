...
class textLine {
  ...
  isRegion() {
      var region = false;
      for (var key in this.tagDict) {        
        if (key.startsWith('Region_') && this.tagDict[key]) {
            region = true;
            break; 
        } 
      }
...
//    this.polygon = new Path2D(p2);
 ...
 isVerticalTopDown() {
   ...
   getAverageXY() {
      ...
      return [parseInt(xSum/this.ptsArray.length), parseInt(ySum/this.ptsArray.length)]
   ...
   getLineAngleVertical(minX, minY, maxX, maxY, max_edges=10){
       
      let maxLength = 0;
      let longestEdgeIndex = 0;
      let polygon = this.ptsArray;
      //Should do it for max of 10 edges
      let step = Math.ceil(polygon.length / max_edges);
    ...
    const p1 = polygon[longestEdgeIndex];
    const p2 = polygon[(longestEdgeIndex + 1) % polygon.length];
    
    var angle = Math.atan2(p2.y - p1.y, p2.x - p1.x) * 180/Math.PI;
    //check valid angle...it should always point downwards
    if (angle > 0 && angle < 180) angle += 180;
    // Now normalize to -180-180 range
   ...
   getLineAngle_y() {
    ...
    if (maxX-minX < maxY-minY) {
        this.isNearVertical = true;
        return this.getLineAngleVertical(minX, minY, maxX, maxY);
    }
    var xArray = [];
    var yArray = [];
  ...
  //Look for the coordinate maxX and minY within rightmost
  //chunk% of line...assume right to left order
  getLineStart(chunk=0.1, rtl=true) {
      
    if (!rtl) {
        alert('Sorting only works for RTL');
    }
    let [minX, MinYOfAll, maxX, maxYOfAll] = this.getCornerPts();
    let minY = 1000000;
    let maxY = 0;

//    let xlimit = maxX - (maxX - minX)  
    let xlimit = maxX - (maxX - minX)*chunk  
    
    for(var i=0; i<this.ptsArray.length; ++i){
      if (this.ptsArray[i].y < minY && 
          this.ptsArray[i].x > xlimit) {
            minY = this.ptsArray[i].y;
      }
      if (this.ptsArray[i].y > maxY &&
          this.ptsArray[i].x > xlimit) {
    ...
    if (minY == 1000000) {
        alert('Sorting not right...minY at 1000000');
        //means line is top to bottom or bottom to top
        //in this case vertical orientation and get a point within upper chunk of line
        minY = MinYOfAll;
    }
    if (maxY == 0) {
        alert('Sorting not right...minY at 1000000');
    ...
    let y_coordinate = parseInt((maxY+minY)/2)
    return [maxX, y_coordinate]; 
...
class manuscriptPage{
  constructor(imageWidth, canvasId="imgCanvas", imageId="manuscript"){
    ...
    this.submit = false;
    this.checked = false;
    this.userType = 'lastEditor';
    this.created = true; // Is the JSON Object empty or not 
    this.timeStamps = {'created':'', 'submitted':'', 'lastEdited': '', 'checked':'', 'users':{}}; //read from json
    //new version
    this.annotators = {'creator':'', 'lastEditor': '', 'transcriber':'', 'transcription_QA': '', 'transcription_tagging': '', 'transcription_tagging_QA': ''}; //read from json
    //old version
    this.taggingBy = '';
    this.transcriber = '';
    this.transcription_QA = '';
    // using this variable for recording time in the updated system
    this.totalTime = 0;
  ...
  initializeLines(linesJson){
    // check if this is empty JSON
    if (Object.keys(linesJson).length > 0) this.created = false;
    if ("timeStamps" in linesJson) this.timeStamps = JSON.parse(JSON.stringify(linesJson.timeStamps));
    if ("annotators" in linesJson) this.annotators = JSON.parse(JSON.stringify(linesJson.annotators));
    
    if ('writer' in linesJson)
        writer.value = linesJson.writer;
    if ('comment' in linesJson)
	comment.value = linesJson.comment;
    if ('documentDate' in linesJson)
	documentDate.value = linesJson.documentDate;
    if ('transcription_QA' in linesJson)
        this.transcription_QA = linesJson.transcription_QA;
    if ('transcriber' in linesJson)
        this.transcriber = linesJson.transcriber;
    if ('taggingBy' in linesJson)
        this.taggingBy = linesJson.taggingBy;
    if ('time' in linesJson)
        this.totalTime = linesJson.time;
    if ('imageContrast' in linesJson)
        this.contrastValue = linesJson.imageContrast;
	
	
    //preserve values for old version
...
//    document.getElementById("log").innerHTML = "ShowAddingLine: line len " + this.drawLine.ptsArray.length;
  ...
  getTimeStamp(seconds) {
    ...
    var usersTimes;
    if ("users" in this.timeStamps) usersTimes = JSON.parse(JSON.stringify(this.timeStamps.users));
    else usersTimes = {};
    var user = ADMIN.user;
    var currentDate = new Date()
    var currentDateStr = currentDate.toISOString().split('.')[0]
    var created, submitted, lastEdited, checked;
    if (this.created) created = currentDateStr;
    else created = this.timeStamps.created;
    lastEdited = currentDateStr;
    if (this.submit) submitted = currentDateStr;
    else submitted = this.timeStamps.submitted;
    if (this.checked) checked = currentDateStr;
    else checked = this.timeStamps.checked;
    // for different users
    
    if (user in usersTimes) usersTimes[user] += seconds;
    else usersTimes[user] = seconds;
    return {'created': created, 'lastEdited': lastEdited, 'submitted': submitted, 
            'checked': checked, 'users': JSON.parse(JSON.stringify(usersTimes))}
  }
  
  getAnnotators(){
    var creator;
    var user = ADMIN.user_fullname;
    if (this.created) creator = user;
    else creator = this.annotators.creator;
    //convert from previous format
    if (this.taggingBy.length != 0) this.annotators.transcription_tagging = this.taggingBy;
    if (this.transcriber.length != 0) this.annotators.transcriber = this.transcriber;
    if (this.transcription_QA.length != 0) this.annotators.transcription_QA = this.transcription_QA;

    // if empty
    if (this.userType in this.annotators && this.annotators[this.userType].length == 0) this.annotators[this.userType] =  user;
	    
    return {'creator':creator, 'lastEditor': user, 
            'transcriber':this.annotators.transcriber, 'transcription_QA': this.annotators.transcription_QA, 
            'transcription_tagging': this.annotators.transcription_tagging, 
            'transcription_tagging_QA': this.annotators.transcription_tagging_QA};
  ...
  getJsonString() {
    ...
    let jsonObj = {'imageFileName': IMAGE_FILENAME, 
                   ...
                   'image_size': {'width': this.imageDim.x, 'height': this.imageDim.y},
                   'imageContrast': this.contrastValue
                   }; 
    
    jsonObj['time'] = this.totalTime + seconds;
    jsonObj['timeStamps'] = this.getTimeStamp(seconds);
    jsonObj['annotators'] = this.getAnnotators();
 ...
 copyClicked() {
   if (this.selectedIndex >= 0)
     this.copiedLine = this.lineArray[this.selectedIndex].createCopy(false, true);
 }

 pasteClicked() {
   if (this.copiedLine == null)
     return;
   let newLine = this.copiedLine.createCopy(false, true);
 ...
 getSortedLineIndices() {
   var lineStarts = new Array(this.lineArray.length);
   for (var i=0;i<this.lineArray.length;++i) {
       lineStarts[i] = this.lineArray[i].getLineStart();
   }
   //sort indices...code by ChatGPT
   const sortedIndices = lineStarts
